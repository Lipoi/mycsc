import dataloaders
from model.modules import ListaParams, Net
import torch
import numpy as np
...
loaders = dataloaders.get_dataloaders(train_path, test_path, patch_size, 1) # 一个图像1patch传入
...
for epoch in tqdm(range(num_epochs)):
    for phase in ['train', 'test']:
        ...
        for batch in loaders[phase]:
            batch = batch.cuda() # batch 是原图像 CBSD432: torch.Size([1, 1, 128, 128])：图像个数，channel数，长，宽 BSD68: 1,1,481,321
            ...
            with torch.set_grad_enabled(phase == 'train'):
                output = model.decoder(noisy_batch)
                loss = (output - batch).pow(2).sum() / batch.shape[0] # 输出-原图像，每个平方再求和再/图像个数。=MSE.注意这里没/每张图片像素个数。所以算总能量要*像素个数shape[2]*shape[3]
            ...
            # statistics
            psnr[phase][epoch] += -10*np.log10(loss.item() / (batch.shape[2]*batch.shape[3])) # noise_std/255, 所以单个像素块最大能量MAX=1
